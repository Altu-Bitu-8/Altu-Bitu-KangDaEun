...
string judge(cards& deck) {
	...
	if (do_deck > su_deck) { // 도도의 덱 사이즈가 더 크면 도도 승리
		return "do";
	}
	else if (do_deck < su_deck) { // 수연의 덱 사이즈가 더 크면 수연 승리
		return "su";
	}
	return "dosu"; // 아니면 비김
}
...
void groundToDeck(deque<int>& deck, deque<int>& ground) {
	while (!ground.empty()) { // 바닥 카드 없어지기 전까지
		deck.push_back(ground.back()); // 덱에 넣기
		ground.pop_back(); // 바닥 카드 없애기
	}
}
...
string game(int m, cards& deck, cards& ground) {
	...
	return judge(deck); // 게임 끝나고 누가 이겼는지 리턴
}
