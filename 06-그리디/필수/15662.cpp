...
{
    // 톱니 번호 인덱스가 0 이상이고 서로 같은 극이 아닐때
    if (current_num - 1 >= 0 && visited[current_num - 1] == 0)
    {
        if (li[current_num][6] != li[current_num - 1][2])
        { // 맞닿아 있는 면이 서로 다른 극인지 체크
            stack.push_back(make_pair(current_num - 1, current_dir * -1)); // pair 스택에 저장함
            visited[current_num - 1] = 1; // 번호 인덱스 값을 1로 변경
            check_rotation(current_num - 1, current_dir * -1); // 다른극이면 해당 톱니도 회전
        }
    }
    // 위와 반대
...
int main()
{
    ...
    { // k번 돌면서 회전할 수 있는지에 대해 체크
        ...
        visited[num - 1] = 1; // 돌린 톱니바퀴 인덱스 값 1로 바꿈
