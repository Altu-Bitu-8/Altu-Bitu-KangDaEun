...
void backtracking(int cnt, int n, int curr_city, int sum, vector<bool>& check, vector<vector<int>>& cost) {
    ...
    if (cnt == n) {
        if (cost[curr_city][0] != 0) {  // 출발 도시로 다시 돌아갈 수 있는가?
            answer = min(answer, sum + cost[curr_city][0]); // 둘 중 더 작은 것을 answer에 넣는다
        }
        return;
    }

    for (int next_city = 0; next_city < n; next_city++) {
        if (check[next_city] || cost[curr_city][next_city] == 0) { // 방문한 적이 있거나 같은 도시일 경우
            continue;
        }

        check[next_city] = true; // 도착
        backtracking(cnt + 1, n, next_city, sum + cost[curr_city][next_city], check, cost); // 다시 실행
