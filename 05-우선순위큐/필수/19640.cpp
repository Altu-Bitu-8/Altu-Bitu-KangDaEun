...
struct cmp {
    bool operator()(const info& child, const info& parent) {
        if (parent.work_days != child.work_days) { // 근무일자 같지 않으면
            return parent.work_days > child.work_days; // 근무일자 더 큰 사람 먼저
        }
        if (parent.urgency != child.urgency) { // 긴급도 같지 않으면
            return parent.urgency > child.urgency; // 긴급도 더 큰 사람 먼저
        }
        return parent.line_num < child.line_num; // 줄 번호 작은 사람 먼저
    }
};
...
int simulation(int m, int k, vector<queue<info>>& lines) {
    ...
    // 우선순위 큐 초기화
    for (int i = 0; i < m; i++) {
        if (!lines[i].empty()) { // 큐가 비어있지 않으면
            pq.push(lines[i].front()); // 최상위 데이터 우선순위 큐에 푸쉬
            lines[i].pop(); // 값 제거
        }
    }
    ...
    while (!pq.empty() && pq.top().idx != k) {  // k의 차례가 되기 전까지
        int line_num = pq.top().line_num; // 줄 번호 받아오기
        pq.pop(); // 제일 처음 사람 날리기

        if (!lines[line_num].empty()) { // line_num에 해당되는 줄 있으면
            pq.push(lines[line_num].front()); // 그 줄에 제일 앞사람 가져오기
            lines[line_num].pop(); // 그 줄 날리기
        }
...
int main() {
    ...
    vector<queue<info>> lines(m, queue<info>()); // 각 줄을 큐로 구현 
